...
const rules = {
  // User profiles - users can view/write their own profile
  userProfiles: {
    allow: {
      view: 'true', // Everyone can view profiles (for display purposes)
  ...
  // Projects - users can view/write their own projects
  projects: {
    allow: {
      view: "data.ownerId == auth.id", // Users can only view their own projects
  ...
  workspaces: {
    allow: {
      view: 'true', // Allow viewing for now (can restrict to members later)
  ...
  workspaceMembers: {
    allow: {
      view: 'true', // Allow viewing for now
  ...
  // Prompt usage - users can view their own usage
  promptUsages: {
    allow: {
      view: "data.userId == auth.id", // Users can only view their own prompt usage
  ...
  subscriptionEvents: {
    allow: {
      view: "data.userId == auth.id", // Users can only view their own subscription events
